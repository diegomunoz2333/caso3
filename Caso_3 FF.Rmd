...
nombres_columnas <- c(
  "País", "Código", "PIB per cápita (US$)", "Población", 
...
tabla_inicial <- datatable(
  Base_2022,
  extensions = c('Buttons', 'Scroller', 'Responsive', 'FixedHeader', 'ColReorder'),
  options = list(
    dom = '<"top"Bfl>rt<"bottom"ip><"clear">',
    buttons = list(
      list(extend = 'copy', text = '📋 Copiar', className = 'btn-cyber-copy'),
      list(extend = 'csv', text = '📊 CSV', className = 'btn-cyber-csv'),
      list(extend = 'excel', text = '📈 Excel', className = 'btn-cyber-excel'),
      list(extend = 'pdf', text = '📄 PDF', className = 'btn-cyber-pdf'),
      list(extend = 'print', text = '🖨️ Imprimir', className = 'btn-cyber-print'),
      list(extend = 'colvis', text = '👁️ Columnas', className = 'btn-cyber-colvis')
    ),
    pageLength = 12,
    ...
    autoWidth = FALSE,
    columnDefs = list(
      list(width = '120px', targets = 0),  # País
      list(width = '80px', targets = 1),   # Código
      list(width = '100px', targets = 2),  # PIB per cápita
      list(width = '100px', targets = 3),  # Población
      list(width = '90px', targets = 4:19) # Resto de variables
    ),
    language = list(
  ...
  rownames = FALSE,
  filter = 'top',
  class = c('display', 'compact', 'hover', 'stripe', 'row-border'),
  caption = htmltools::tags$caption(
    style = 'caption-side: top; text-align: center; color: #00e5ff; font-size: 18px; font-weight: bold; margin-bottom: 15px;',
    '🌍 DATASET COMPLETO - 17 VARIABLES DE DESARROLLO GLOBAL 2022'
  ),
  ...
  htmlwidgets::onRender("
    function(el, x) {
      ...
      // ESTILIZAR ENCABEZADOS PRINCIPALES
      ...
      $(el).find('tbody tr').hover(
        function() {
          $(this).css({
            'background': 'linear-gradient(90deg, rgba(0, 229, 255, 0.1) 0%, rgba(10, 14, 26, 0.9) 100%) !important',
            'box-shadow': 'inset 0 0 30px rgba(0, 229, 255, 0.3) !important',
      ...
      var filterRow = $(el).find('thead tr:eq(1)');
      filterRow.css({
        'background': 'linear-gradient(135deg, #12161f 0%, #0a0e1a 100%) !important',
      ...
      filterRow.find('input').css({
        ...
        'width': '90% !important',
        'box-shadow': 'inset 0 0 15px rgba(0, 229, 255, 0.3) !important',
        'font-weight': 'bold'
      }).attr('placeholder', '🔍 Filtrar...');
      
      // BOTONES DE EXPORTACIÓN - MUY VISIBLES
      var buttonContainer = $(el).find('.dt-buttons');
      buttonContainer.css({
        'margin-bottom': '15px !important',
        'text-align': 'center !important'
      });
      ...
      $(el).find('.dt-button').css({
        ...
        'transition': 'all 0.3s ease !important',
        'box-shadow': '0 4px 15px rgba(0, 229, 255, 0.6) !important',
        ...
        function() {
          $(this).css({
            ...
            'box-shadow': '0 6px 25px rgba(0, 229, 255, 0.9) !important',
      ...
      // PAGINACIÓN MEJORADA
      $(el).find('.dataTables_paginate .paginate_button').css({
        'background': 'linear-gradient(135deg, #12161f 0%, #0a0e1a 100%) !important',
        'color': '#e0e6ed !important',
        'border': '2px solid #00e5ff !important',
      ...
      $(el).find('.dataTables_paginate .paginate_button.current').css({
        'background': 'linear-gradient(135deg, #00e5ff 0%, #0084ff 100%) !important',
        'color': '#000000 !important',
...
p1 <- plot_ly(var_df,
  ...
  layout(
    title = list(
      text = "<b style='color:#00e5ff; font-size:20px;'>Variables: Dimensión 1 vs Dimensión 2</b><br>
...
p2 <- plot_ly(var_df,
  ...
  layout(
    title = list(
      text = "<b style='color:#00e5ff; font-size:20px;'>Variables: Dimensión 3 vs Dimensión 4</b><br>
...
p1 <- plot_ly() %>%
  ...
  layout(
    title = list(
      text = "<b style='color:#00e5ff; font-size:22px;'>Biplot: Dimensión 1 vs Dimensión 2</b><br>
...
p2 <- plot_ly() %>%
  ...
  # Marcadores de países (azul oscuro)
  add_trace(
    data = ind_df,
    x = ~Dim3, y = ~Dim4,
    text = ~paste("<b style='color:#00e5ff; font-size:14px;'>", Pais, "</b><br>",
  ...
  layout(
    title = list(
      text = "<b style='color:#00e5ff; font-size:22px;'>Biplot: Dimensión 3 vs Dimensión 4</b><br>
             <span style='color:#8b92a0; font-size:13px;'></span>",
      x = 0.5,
      xanchor = 'center',
      y = 0.98
