...
# Calcular la suma de cuadrados within (WCSS) para diferentes números de clusters
wss <- function(k) {
...
elbow_plot <- ggplot(elbow_data, aes(x = k, y = wss)) +
  geom_line(size = 1.2, color = "#2E9FDF") +
  geom_point(size = 3, color = "#2E9FDF") +
  geom_vline(xintercept = 3, linetype = "dashed", color = "#FC4E07", size = 1) +
  labs(
    title = "Método del Codo para Determinar Número Óptimo de Clusters",
    subtitle = "Análisis basado en la suma de cuadrados within-cluster (WCSS)",
...
# Versión interactiva
elbow_plot_interactive <- ggplotly(elbow_plot) %>%
  layout(
    title = list(
      text = "<b>Método del Codo para Determinar Número Óptimo de Clusters</b>",
      x = 0.05
    ),
    annotations = list(
      list(
