...
<!-- Mini TOC Derecho -->
<div id="right-toc">
  <div id="right-toc-title">EN ESTA SECCIÓN</div>
  <ul id="right-toc-list"></ul>
</div>

<!-- Botón Control de Código -->
<button id="global-code-toggle"> Código</button>
...
2. **Identificar grupos** de países con características similares
3. **Interpretar** los patrones encontrados en el contexto del desarrollo internacional
4. **Proporcionar insights** para políticas públicas y análisis económico
...
### Gráfico de Burbujas: Economía Global

Análisis multidimensional de la economía mundial 2022. Este gráfico permite 
visualizar simultáneamente cuatro dimensiones: PIB per cápita (eje X), crecimiento 
económico (eje Y), tamaño poblacional (tamaño de burbuja) y ubicación geográfica 
(color por continente).

```{r grafico-burbujas-COMPLETO, fig.height=7, fig.width=11}
...
# Preparar datos con burbujas PEQUEÑAS y proporcionales
datos_burbujas <- Base_2022 %>%
  asignar_continentes() %>%
  filter(!is.na(PIB_per) & !is.na(`Crecimiento PIB`) & !is.na(Continente)) %>%
  select(Pais, Continente, PIB_per, `Crecimiento PIB`, Poblacion) %>%
  mutate(
    # Tamaño: logaritmo para evitar burbujas enormes
    Tamaño = log10(Poblacion),
    
    tooltip_text = paste0(
      "<b>", Pais, "</b><br>",
      Continente, "<br>",
      "PIB: $", round(PIB_per), "<br>",
      "Crec: ", round(`Crecimiento PIB`, 2), "%<br>",
      "Población: ", round(Poblacion/1e6, 1), "M"
    )
  )
...
plot_ly(
  ...
  marker = list(
    sizeref = 2 * max(datos_burbujas$Tamaño) / 15^2,
    sizemin = 3,
    opacity = 0.65,
    line = list(color = '#00e5ff', width = 0.8)
  ),
  showlegend = TRUE
) %>%
  layout(
    ...
    title = list(
      text = "<b style='color:#00e5ff; font-size:18px;'>Economía Global: PIB per Cápita vs Crecimiento</b><br>
               <span style='color:#8b92a0; font-size:12px;'>Tamaño burbuja = Población | Año 2022</span>",
      x = 0.5, xanchor = 'center'
    ),
    xaxis = list(
      title = "<b>PIB per cápita (USD)</b>",
      type = 'linear',
      gridcolor = '#12161f',
      showgrid = TRUE,
      tickcolor = '#8b92a0'
    ...
    yaxis = list(
      title = "<b>Crecimiento PIB Anual (%)</b>",
      gridcolor = '#12161f',
      showgrid = TRUE,
      tickcolor = '#8b92a0'
    ),
...
```{r validar-grafico-completo}

datos_check <- Base_2022 %>% asignar_continentes() %>% 
  filter(!is.na(Continente) & !is.na(PIB_per) & !is.na(`Crecimiento PIB`))

cat("✅ Total países:", nrow(datos_check), "\n")
cat("📊 PIB rango: $", round(min(datos_check$PIB_per)), " - $", round(max(datos_check$PIB_per)), "\n")
cat("📈 Crec rango: ", round(min(datos_check$`Crecimiento PIB`), 2), "% - ", round(max(datos_check$`Crecimiento PIB`), 2), "%\n")

# Por continente
stats <- datos_check %>% group_by(Continente) %>% summarise(Paises = n(), .groups = 'drop')
cat("\nPor continente:\n")
for(i in 1:nrow(stats)) {
  cat("- ", stats$Continente[i], ": ", stats$Paises[i], " países\n")
}
...
<div class="footer">
  <strong>Universidad del Valle</strong> | <em>Facultad de Ingeniería</em> | Gestión de Datos<br>
  Profesor: Orlando Joaqui Barandica | `r format(Sys.Date(), '%Y')`
</div>