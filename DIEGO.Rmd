...
El criterio de selección del número de componentes a retener se basó en el criterio de varianza acumulada $\geq 70\%$, identificando el número mínimo de componentes $k$ que satisface:

$$
\frac{\sum_{i=1}^{k} \lambda_i}{\sum_{j=1}^{p} \lambda_j} \geq 0.70
$$

Este umbral del $70\%$ representa un balance metodológicamente establecido entre dos objetivos contrapuestos: maximizar la compresión dimensional (retener información en pocas variables sintéticas) y minimizar la pérdida de información (descartar dimensiones relevantes) \cite{Kaiser1960, Hastie2009}.

Un umbral más bajo (ej. $60\%$) introduciría pérdida excesiva de estructura; uno más alto (ej. $85\%$) reintroduciría la alta dimensionalidad que se busca evitar \cite{Jolliffe2002}.
...
tabla_pc4 <- crear_tabla_contribucion(4, contribuciones, cos2_df,
                                     ...
                                     9)
tabla_pc5 <- crear_tabla_contribucion(5, contribuciones, cos2_df,
                                     "Contribución de Variables al Componente Principal 5 (6.3% de varianza)",
                                     10)
tabla_pc6 <- crear_tabla_contribucion(6, contribuciones, cos2_df,
                                     "Contribución de Variables al Componente Principal 6 (5.2% de varianza)",
                                     11)
...
Mientras que San Marino contribuye en la dimensión 2 con un 0%, contrariamente de su aporte en la dimensión 1 el cual es 2.86%, lo cual refiere a qué este país, tiene una baja tasa económica mientras que en factores como esperanza de vida es muy fuerte.
...
Al observar el grafico podemos identificar algunos datos, casos como la India donde su aporte en la dimensión 4 es de 8,19% lo cual nos demuestra su gran tasa poblacional y de tierra cultivable.
Timor-leste tiene un aporte en la dimensión 4 de un 12,88% el cual refiere que tiene poca tendencia de uso de internet y un alta área boscosa.
Otro caso a referir Djibouti tiene dos valores importantes en la dimensión 3 tiene un aporte de 8,47% y en la 4 de 0,24%, lo cual refiere que es uno de los paises que menos exporta y más importa.
...
dimensiones_tabla <- tribble(
  ...
  "Dimensión 1", "Desarrollo humano y modernización tecnológica", 
  "Esperanza.vida, Uso.internet, Acceso.electricidad, PIB.per.capita, Mortalidad.infantil", 
  "Noruega (Alta, Alta, Alta, Alta, Baja); Chad (Baja, Baja, Baja, Baja, Alta); Colombia (Media, Media, Media, Media, Media)",
  
  "Dimensión 2", "Ruralidad y dependencia de remesas vs industrialización", 
  "Remesas, Tierra.cultivable, Industria, Crecimiento.poblacion, PIB.per.capita", 
  "Honduras (Alta, Alta, Baja, Baja, Baja); Alemania (Baja, Baja, Alta, Alta, Alta); Ecuador (Media, Media, Media, Media, Media)",
  
  "Dimensión 3", "Apertura comercial e inversión extranjera", 
  "Importaciones, Exportaciones, Inversion.extranjera, Area.boscosa, Poblacion", 
  "Singapur (Alta, Alta, Alta, Baja, Baja); Bután (Baja, Baja, Baja, Alta, Baja); Costa Rica (Media, Media, Media, Media, Media)",
  
  "Dimensión 4", "Uso del suelo: intensificación agrícola y presión poblacional", 
  "Tierra.cultivable, Area.boscosa, Poblacion, Crecimiento.PIB", 
  "India (Alta, Baja, Alta, Media); Finlandia (Baja, Alta, Baja, Baja); Argentina (Media, Media, Media, Media)"
)
...
- $x_i$ es la posición de la observación $i$ en el espacio de componentes principales,
- $\bar{c}$ es el centroide (promedio) del cluster $c$.
...
coordenadas_componentes <- resultado_ACP$dudi$li[, 1:6]
